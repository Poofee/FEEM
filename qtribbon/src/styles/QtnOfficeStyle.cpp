...
/*!
    \brief Sets the flag for drawing MDI window title with native style or with office/ribbon style.
*/ 
...
/*!
    \brief Returs the flag for drawing MDI window title with native style or with office/ribbon style.
*/ 
...
void OfficeStyle::polish(QWidget* widget)
{
    ...
    else if (!isDialogsIgnored() && widget->inherits("QDialog"))
    {
        QPalette palette = widget->palette();
    ...
    else if(widget->inherits("QStatusBar"))
    {
        QPalette palette = widget->palette();
        palette.setColor(QPalette::Foreground, d.m_clrStatusBarText);
        widget->setPalette(palette);
    }
    else if(widget->inherits("QMdiArea"))
    {
    ...
    else if (widget->inherits("QTabBar") && qobject_cast<QMdiArea*>(widget->parentWidget()))
    {
        widget->setAutoFillBackground(true);
        QPalette palette = widget->palette();
    ...
    if(defaultStyle())
    {   
        if (d.m_ignoreMDIWindowTitle && widget->inherits("QMdiSubWindow"))
        ...
        case CC_TitleBar:
            {
                ...
                else if (/*widget && widget->inherits("QMdiSubWindow")*/true)
                {
...
int OfficeStyle::styleHint(StyleHint hint, const QStyleOption* opt, const QWidget* widget, QStyleHintReturn* returnData) const
{
    QTN_D(const OfficeStyle);
    int ret = 0;
    if (d.officePaintManager()->styleHint(hint, opt, widget, returnData, ret))
        return ret;
...
bool OfficeStyle::drawToolButtonLabel(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    {
        if (!w->inherits("Qtitan::PopupColorButton"))
            if (w->parent() && !w->parent()->inherits("Qtitan::RibbonQuickAccessBar"))
                return false;
    }
        ...
        else 
        {
            ...
            if (w && w->inherits("Qtitan::RibbonButton"))
            {
                if (!pmSize.isEmpty())
                {
                    ...
                    int iconExtent = index != -1 ?  toolbutton->icon.actualSize(lSz[index]).height() : proxy()->pixelMetric(PM_SmallIconSize);
