...
void DsgnRibbonBarContainer::addWidget(QWidget* widget)
{
    if (RibbonPage* ribbonPage = qobject_cast<RibbonPage*>(widget))
    {
        ...
        if (RibbonTabBar* ribbonTabBar = m_pRibbon->findChild<Qtitan::RibbonTabBar*>())
            setObjectNameChildren(*ribbonTabBar);
        Q_ASSERT(QDesignerFormWindowInterface::findFormWindow(m_pRibbon) != 0);
        QDesignerFormWindowInterface::findFormWindow(m_pRibbon)->manageWidget(ribbonPage);
        // set current page
