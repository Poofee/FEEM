...
/*!
    \enum Qtitan::RibbonBar::QuickAccessBarPosition
...
RibbonBarPrivate::RibbonBarPrivate()
{
    ...
    m_countKey = 0;
    m_wheelEvent = false;
    m_alignmentLogotype = Qt::AlignRight;
#ifdef CUSTOMIZE_V4_WORK
    m_customizationManager = Q_NULL;
...
void RibbonBarPrivate::init()
{
    ...
    m_quickAccessBar->setObjectName(QLatin1String("qtn_Ribbon_Quick_Access_Bar"));
    m_quickAccessBar->setVisible(false);

#ifdef CUSTOMIZE_V4_WORK
    m_customizationManager = new RibbonCustomizationManager(&p);
#endif // CUSTOMIZE_V4_WORK
}
...
//    const int heightCaptionGroup = p.style()->pixelMetric((QStyle::PixelMetric)RibbonStyle::PM_RibbonHeightCaptionGroup, &opt, 0);
    ...
    if (m_quickAccessBar->isVisible() && m_quickAccessBarPosition != RibbonBar::QATopPosition)
    {
        if (m_minimized)
    ...
    if (m_controlSystemButton)
    {
        ...
        QRect rcControl(QPoint(left, top), szControl);
    ...
    {
//      left = m_rcQuickAccess.left() + (!m_quickAccessToolbarOnTop ? 2 : 0);
    }

    if (left + 6 > right || !m_tabBar->isVisible())
    ...
    QRect rectLogotype = m_rectLogotype;
    if (!rectLogotype.isNull() && m_alignmentLogotype == Qt::AlignLeft)
    ...
    if (validIndex(index))
    {
        ...
        QSize sz = m_rcPageArea.size();
        if (!sz.isEmpty())
            page->setFixedSize(sz);

    }

    layoutTitleButton();
    layoutBackstageCloseButton();
    ...
    if (!m_pxLogotype.isNull())
    {
        ...
        if (validIndex(index))
        {
            ...
            {
                int width = m_pxLogotype.width();
                int height = m_pxLogotype.height();
                QRect rcPage = page->geometry();

                if (height < rcPage.height())
                    m_rectLogotype.setTop(rcPage.top() + (rcPage.height()-height)/2);
                else
                {
                    m_rectLogotype.setTop(0);
                    height = rcPage.height();
                }

                if ( m_alignmentLogotype == Qt::AlignRight )
                    m_rectLogotype.setLeft(rcPage.right() - width);

                m_rectLogotype.setWidth(width);
                m_rectLogotype.setHeight(height);
            }
        }
    }
}
...
void RibbonBarPrivate::layoutTitleButton()
{
    ...
    m_rcHeader.adjust(0, frameBorder, 0, /*-3*/0);
...
void RibbonBarPrivate::layoutBackstageCloseButton()
{
    QTN_P(RibbonBar);
    if (p.isBackstageVisible() && (bool)p.style()->styleHint((QStyle::StyleHint)RibbonStyle::SH_RibbonBackstageHideTabs))
    {
        RibbonBackstageView* backstage = qobject_cast<RibbonBackstageView*>(m_controlSystemButton->backstage());
        Q_ASSERT(backstage != Q_NULL);

        if (!m_backStageCloseButton)
        {
            m_backStageCloseButton = new RibbonBackstageCloseButton(&p);
            ...
            QObject::connect(m_backStageCloseButton, SIGNAL(pressed()), backstage, SLOT(close()));
        ...
        backstage->initStyleOption(&opt);

        m_rcHeader.setTopLeft(QPoint(m_rcHeader.top() + opt.menuWidth, m_rcHeader.top()));

        QRect rectClose(QPoint(0, 0), QSize(opt.menuWidth, p.backstageViewTop()));
        m_backStageCloseButton->setGeometry(rectClose);
    }
    else
    {
        if (m_backStageCloseButton)
...
/*      // Fixed: pages with the same context information (color and title), the context title is not drawn upon the tabs of the pages.
        if (prevContextHeader && prevContextHeader->color == tab->contextColor() &&
...
*/
        {
            if (isExistAssociativeTabWithPage(tab))
            {
...
void RibbonBarPrivate::currentChanged(int index)
{
    ...
    if (validIndex(index))
    {
        ...
        else
        {
            ...
            {
                page->setFixedSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX);
                ...
                page->popup();
                QRect rc(pnt, QSize(p.sizeHint().width(), height));
                page->setGeometry(rc);
...
void RibbonBarPrivate::titleBarOptions(StyleOptionTitleBar& titleBarOptions) const
{
    ...
    titleBarOptions.quickAccessVisibleCount = m_quickAccessBar ? m_quickAccessBar->visibleCount() : 0;
...
QMenu* RibbonBarPrivate::createContextMenu()
{
    ...
    popup->setObjectName(QLatin1String("qtn_ribbon_menu"));
    ...
    if (p.isQuickAccessVisible())
    {
        if (RibbonQuickAccessBar* quickAccessBar = p.quickAccessBar())
        {
            ...
            if (actions.size() > 0)
            {
                action = popup->addAction(QObject::tr("Customize Quick Access Toolbar..."));
                action->setObjectName(strCustomizeQAToolBar);
                //                connect(action, SIGNAL(triggered()), &qtn_d(), SLOT(toggledQuickAccessToolbarOnTop()));

                action = popup->addAction(p.quickAccessBarPosition() == RibbonBar::QATopPosition ? QObject::tr("Show Quick Access Toolbar Below the Ribbon") 
    ...
    {
        popup->addSeparator();
        action = popup->addAction(QObject::tr("Minimize the Ribbon"));
...
int RibbonBarPrivate::maxHeightPages() const
{
    QTN_P(const RibbonBar);
    QStyleOption opt;
...
void RibbonBar::setLogoPixmap(Qt::AlignmentFlag alignment, const QPixmap& logotype)
{
    ...
    if ( alignment == Qt::AlignRight || alignment == Qt::AlignLeft)
    {
        ...
        d.m_alignmentLogotype = alignment;
        d.m_pxLogotype = logotype;
        return;
    }
    Q_ASSERT(false);
}
...
*/
const QPixmap& RibbonBar::logoPixmap(Qt::AlignmentFlag& alignment) const
{
    QTN_D(const RibbonBar);
    alignment = d.m_alignmentLogotype;
    return d.m_pxLogotype;
}
...
void RibbonBar::setMinimizedFlag(bool flag)
{
    ...
    if (d.m_minimized != flag)
    {
        ...
        else
        {
            for (int i = 0, count = d.m_listPages.size(); count > i; i++)
                d.updateMinimizedModePage(d.m_listPages.at(i));
            d.currentChanged(currentIndexPage());
        }
...
*/ 
#ifdef CUSTOMIZE_V4_WORK
RibbonCustomizationManager* RibbonBar::customizeManager() const
{
    QTN_D(const RibbonBar);
    return d.m_customizationManager;
}
#endif // CUSTOMIZE_V4_WORK
...
/*!
\brief Returns the number of the pages inside ribbon bar.
*/ 
...
{
    return titleBarHeight() + topBorder() + tabBarHeight(true);
}
...
int RibbonBar::titleBarHeight() const
{
    ...
    if (!isQuickAccessVisible() || d.m_quickAccessBarPosition != RibbonBar::QATopPosition)
    {
//        if (parentWidget()->windowState() & Qt::WindowMaximized)
//            height += topBorder() * 0.5;
        return height;
    }

    int quickAccessHeight = quickAccessBar()->height();
...
{
    QTN_D(const RibbonBar);
    return d.m_frameHelper && d.m_frameHelper->isDwmEnabled() ? d.m_frameHelper->frameBorder() : 0;
}
...
void RibbonBar::paintEvent(QPaintEvent* event)
{
    ...
    {
        opt.logotype = d.m_pxLogotype;
        opt.rectLogotype = d.m_rectLogotype;
        ...
        case QEvent::FontChange: 
            {
                ...
                if (QWidget* wd = this->parentWidget())
                    QApplication::postEvent(wd, new QEvent(QEvent::StyleChange));
            }
            break;
        default:
            break;
...
void RibbonBar::contextMenuEvent(QContextMenuEvent* event)
{
    ...
    if (QMenu* menu = d.createContextMenu()) 
    {
        ...
        bool showMenu = false;
        emit showRibbonContextMenu(menu, showMenu);
