...
void GroupDefaultLayout::insertAction(int index, QAction* action)
{
    ...
    if (GroupItem* item = createItem(action)) 
    {
...
void GroupDefaultLayout::setGeometry(const QRect& rect)
{
    ...
    Q_ASSERT(qobject_cast<RibbonGroup*>(parentWidget()) != Q_NULL);
    resetLayout(true);
...
void GroupDefaultLayout::widthAdjustment()
{
    QRect rect = geometry();
    for (int i = items.count() - 1; i >= 0; i--)
    {
        GroupItem* item = items.at(i);
        if (item->widths.size() > 0)
        {
            for (int w = item->widths.count() - 1; w >= 0; w--)
            {
                QRect rectItem = item->rcItem;
                rectItem.setWidth(item->widths[w]);

                if (rectItem.right() < rect.right())
                {
                    item->rcItem = rectItem;
                    item->szItem = rectItem.size();
                }
            }
        }
    }
...
void GroupDefaultLayout::updateAlignWidget(QList<ExWidgetWrapper*>& alignWrappers) const
{
    ...
    if (count > 1)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            maxWidthTotal = qMax(maxWidthTotal, wrapper->sizeHintLabels());
        }
        for (int i = 0; i < count; i++)
            alignWrappers.at(i)->setLengthLabel(maxWidthTotal);
    }
}
...
void GroupDefaultLayout::updateCenterColumn(int firstItem, int lastItem, int groupHeight) const
{
    ...
    int nOffset = (groupHeight - height) / (nItems + 1);
...
void GroupDefaultLayout::updateGeomArray_() const
{
    ...
    if (rowCount == 1)
    {
        ...
        else if (!(allow3Row && m_rowCount == 3 && findBestWrapSpecialDynamicSize_()))
        {
...
void GroupDefaultLayout::reposition(const QRect& rc)
{
    int dataCount = items.count();
    ...
    for (int i = dataCount - 1; i >= 0; i--)
    {
        ...
        if (RibbonGallery* widgetGallery = qobject_cast<RibbonGallery*>(item->widget()))
        {
            if (widgetGallery->isResizable())
            {
                ...
                QApplication::sendEvent(widgetGallery, &ev);

                QRect rect = rc;
                QSize rectOld = item->widget()->baseSize();
                int widthAvail = rect.width();

                if (rectOld.width() < widthAvail)
                    widthAvail = rectOld.width();
                ...
                if (cx > 0 && cx <= widthAvail)
                {
                    ...
                    item->szItem = QSize();
                    QSize rcWd = item->widget()->baseSize();
                    int width = qMin(maxWidth, int(widthAvail / cx) * cx);
                    width += rcBorders.width() + 2;
                    if (m_lastWidth != width)
                    {
                        m_lastWidth = width;
                        rcWd.setWidth(width);
                        item->widths << width;
                        item->widget()->setBaseSize(rcWd);
                        updateGeomArray();
                        item->szItem = QSize();
                        item->widget()->setBaseSize(rectOld);
                    }
                }
            }
        }
...
void GroupDefaultLayout::resetLayout(bool updateGeom)
{
    for (int i = 0; i < items.count(); i++)
    {
        GroupItem* item = items.at(i);
...
void GroupDefaultLayout::cleanUpSizes()
{
    for (int i = 0; i < items.count(); i++)
    {
        GroupItem* item = items.at(i);
...
RibbonGroupPrivate::RibbonGroupPrivate()
{
    ...
    m_butOption = Q_NULL;
    m_actOption = Q_NULL;
    m_ribbonDefaultGroupButton = Q_NULL;
    m_reduced = false;
...
RibbonGroupPrivate::~RibbonGroupPrivate()
{
    delete m_ribbonDefaultGroupButton;
    m_ribbonDefaultGroupButton = Q_NULL;
}
...
void RibbonGroupPrivate::resetLayout(bool updateGeom)
{
    QTN_P(RibbonGroup)
    if (GroupDefaultLayout* gLayout = qobject_cast<GroupDefaultLayout*>(p.layout()))
        gLayout->resetLayout(updateGeom);
}

int RibbonGroupPrivate::minWidth() const
{
    QTN_P(const RibbonGroup)
    QStyleOption opt;
    opt.init(&p);
...
void RibbonGroupPrivate::resetGeometry()
{
    QTN_P(RibbonGroup)
    resetLayout(false);
...
void RibbonGroupPrivate::recalcWidths(int height)
{
    Q_UNUSED(height);
    QTN_P(RibbonGroup)
    ...
    bool reduced = m_reduced;
    m_reduced = true;
    RibbonDefaultGroupButton* ribbonDefaultGroupButton = new RibbonDefaultGroupButton(p.parentWidget(), &p);
    ribbonDefaultGroupButton->resize(ribbonDefaultGroupButton->sizeHint());
    const int cxDefaultButton = ribbonDefaultGroupButton->sizeHint().width();
    delete ribbonDefaultGroupButton;
    m_reduced = reduced;

    if (m_controlsGrouping)
    {
        if (GroupDefaultLayout* gLayout = qobject_cast<GroupDefaultLayout*>(p.layout()))
        {
            const int maxRows = m_maxRows;
            for (int rows = gLayout->m_rowCount; rows <= maxRows; rows++)
            {
                QRect rect(QPoint(0, 0), QSize(cxDefaultButton + 1, height));
                ...
                for (;; right += dx)
                {
                    ...
                    {
                        if (m_widths.size() == 0)
                            m_widths.append(32767);
                        break;
                    }

                    gLayout->m_rowCount = rows;
                    resetGeometry();
                    int fullWidth = p.sizeHint().width();

                    if (lastWidth != -1 && fullWidth > lastWidth)
                        break;

                    if (gLayout->m_rowCount == rows && fullWidth > 0)
                    {
                        // Don't append 1 row layout:
                        if (gLayout->m_rowCount == maxRows - 1)
                            m_widths.clear();

                        m_widths.append(fullWidth);
                        lastWidth = fullWidth;
                        break;
                    }
                }
            }
            gLayout->m_rowCount = 2;
        }
    }
    else
    {
        if (GroupLayout* gLayout = qobject_cast<GroupLayout*>(p.layout()))
        {
            gLayout->cleanUpSizes();
            QRect rect(QPoint(0, 0), QSize(32767, height));
            do
            {
                resetLayout();
                gLayout->reposition(rect);
                ...
                if (lastWidth == -1 || fullWidth < lastWidth)
                {
                    ...
                    if (lastWidth <= cxDefaultButton || (lastWidth <= 3 * cxDefaultButton / 2 && gLayout->count() == 1))
                    {
                        if (m_widths.size() == 0)
                        {
                            // Group has onle one layout and it smaller then reduced.
                            // Use this layout only and don't allow to reduce the group
                            m_widths.append(lastWidth);
                            return;
                        }
                        break;
                    }
            ...
            while (rect.width () > 2 * m_xMargin);
        }
    }
    m_widths.append(cxDefaultButton);
}
...
void RibbonGroupPrivate::reposition(const QRect& rect)
{
    ...
    if (m_reduced)
    {
        if (!m_ribbonDefaultGroupButton)
        {
            m_hasVisible = p.isVisible();
            p.setVisible(false);

            p.setWindowFlags(Qt::Popup);

            m_ribbonDefaultGroupButton = new RibbonDefaultGroupButton(p.parentWidget(), &p);
            m_ribbonDefaultGroupButton->setIcon(m_icon);
            m_ribbonDefaultGroupButton->resize(m_ribbonDefaultGroupButton->sizeHint());
            m_ribbonDefaultGroupButton->setVisible(true);
            p.setFocusPolicy(Qt::NoFocus);
            p.setFocusProxy(m_ribbonDefaultGroupButton);
            p.setUpdatesEnabled(true);
            connect(&p, SIGNAL(actionTriggered(QAction*)), this, SLOT(hidePopupMode(QAction*)));
        ...
        m_ribbonDefaultGroupButton->setGeometry(rect);
    }
    else
    {
        if (m_ribbonDefaultGroupButton)
        {
            m_ribbonDefaultGroupButton->setVisible(false);
            m_ribbonDefaultGroupButton->deleteLater();
            m_ribbonDefaultGroupButton = Q_NULL;
            ...
            p.setVisible(m_hasVisible);
            disconnect(&p, SIGNAL(actionTriggered(QAction*)), this, SLOT(hidePopupMode(QAction*)));
            m_hasVisible = false;
        }
        ...
        {
            if (GroupDefaultLayout* gLayout = qobject_cast<GroupDefaultLayout*>(p.layout()))
                gLayout->m_rowCount = m_currWidthIndex == 0 ? 2 : 3;
        }
...
bool RibbonGroup::isVisible() const
{
    QTN_D(const RibbonGroup)
    return QWidget::isVisible() || d.m_hasVisible;
}

void RibbonGroup::setVisible(bool visible)
{
    QTN_D(RibbonGroup)
    if (d.m_ribbonDefaultGroupButton && !d.m_ribbonDefaultGroupButton->isShowPopup())
    {
        d.m_hasVisible = visible;
        d.m_ribbonDefaultGroupButton->setVisible(visible);
    }
    else if (!QApplication::activePopupWidget() || (QApplication::activePopupWidget() == this) || (visible && !QWidget::isVisible()))
...
void RibbonGroup::setTitle(const QString& title)
{
    ...
    d.m_title = title;
    adjustSize();
    if (d.m_ribbonDefaultGroupButton)
    {
        d.m_ribbonDefaultGroupButton->setText(title);
        adjustSize();
    }
...
void RibbonGroup::setIcon(const QIcon& icon)
{
    ...
    if (d.m_ribbonDefaultGroupButton)
    {
        d.m_ribbonDefaultGroupButton->setIcon(icon);
        adjustSize();
    }
}
...
{
    QTN_D(RibbonGroup);
    if ((windowFlags() & Qt::Popup) && !rect().contains(event->pos()) && d.m_ribbonDefaultGroupButton) 
    {
        QRect rect = d.m_ribbonDefaultGroupButton->rect();
        rect.moveTopLeft(d.m_ribbonDefaultGroupButton->mapToGlobal(rect.topLeft()));
