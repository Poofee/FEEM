    ...
    if (!m_dwmEnabled)
    {
        ...
        else
        {
            ...
            if (opt.titleVisible)
            {
                ...
                QSize szIcon = sizeSystemIcon(QIcon(), QRect());//(GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON));
...
//    QRasterPaintEngine* en = (QRasterPaintEngine *)painter->paintEngine();
    ...
    wchar_t* text = new wchar_t[++len];
    ::GetWindowTextW(m_hwndFrame, text, len + 1);
    drawDwmCaptionText(painter, opt.rcTitleText, text[0] != Q_NULL ? QString::fromWCharArray(text) : opt.text, QColor(), opt.state & QStyle::State_Active);
    delete[] text;
    ...
    if (opt.drawIcon && !opt.icon.isNull()) 
    {
        ...
        if (HICON hIcon = windowIcon(opt.icon, opt.rect))
        {
            ...
            if (m_isTransparentCaption && pDrawThemeIcon && hTheme)
            {
                ...
                HIMAGELIST hImageList = ::ImageList_Create(szIcon.width(), szIcon.height(), ILC_COLOR32 | ILC_MASK, 0, 0);
...
void OfficeFrameHelperWin::drawDwmCaptionText(QPainter* painter, const QRect& rc, const QString& strWindowText, const QColor& colText, bool active)
{
...
{
    HDC hdc = static_cast<HDC>(QGuiApplication::platformNativeInterface()->nativeResourceForBackingStore(QByteArrayLiteral("getDC"), farame->backingStore()));
    *offset = QPoint(0, 0);
            ...
            *result = DefWindowProcW(msg->hwnd, msg->message, msg->wParam, msg->lParam);
            ...
            if (*result == HTCLIENT)
            {
                ...
                if (*result == HTCLIENT)
                {
                    ...
                    {
                        DWORD dwStyle = getStyle();
                        if (pos.x() < 7 + /*GetSystemMetrics(SM_CXSMICON)*/sizeSystemIcon(QIcon(), QRect()).width() && (dwStyle & WS_SYSMENU))
                            *result = HTSYSMENU;
                    }
                }
            }
...
//        m_frameBorder = 3;
        ...
        else
        {
            if (isTheme2013())
            {
                ...
                {
//                    if (m_closeWindow)
//                        lpncsp->rgrc[0].bottom += isSmallSystemBorders() ? 3 : m_frameBorder;
//                    else
                    {
