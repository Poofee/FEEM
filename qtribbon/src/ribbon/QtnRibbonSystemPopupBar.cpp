...
void RibbonSystemPopupBar::paintEvent(QPaintEvent* event)
{
    ...
    {
        QSize sz = QMenu::sizeHint();
        StyleHintReturnThemeColor hintReturn("Ribbon", "MenuPopupSeparator", QColor(197, 197, 197));
        ...
        StyleHintReturnThemeColor hintRectReturn("Ribbon", "MenuPopupRect", QColor());
        const QColor rectColor = static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintRectReturn));
        if (hintRectReturn.isValid)
        {
            QRect rect = QRect(QPoint(d.m_rcBorders.left(), d.m_rcBorders.top()), 
                QSize(sz.width() - hMargin, sz.height() - d.m_rcBorders.top() - d.m_rcBorders.bottom()/2-2) );
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        {
            StyleHintReturnThemeColor hintReturn("Ribbon", "RecentFileListBackground");
            const QColor fillColor = static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintReturn));
            p.fillRect(page->geometry(), fillColor);

            StyleHintReturnThemeColor hintColorReturn("Ribbon", "MenuPopupRect", QColor());
            const QColor rectColor = static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintColorReturn));
            if (hintColorReturn.isValid)
            {
                QRect rect = page->geometry().adjusted(0, 0, 0, 2);
                QPainterPath path;
...
void RibbonPageSystemPopupListCaption::paintEvent(QPaintEvent* event)
{
    ...
    StyleHintReturnThemeColor hintClientText("Ribbon", "GroupClientText");
    ...
    StyleHintReturnThemeColor hintEdgeShadow("Ribbon", "RecentFileListEdgeShadow");
    ...
    StyleHintReturnThemeColor hintEdgeHighLight("Ribbon", "RecentFileListEdgeHighLight");
    ...
    : RibbonPageSystemPopupListCaption(caption)
{
    ...
    if (caption.isEmpty())
        setWindowTitle(QObject::tr("Recent Documents"));
...
void RibbonPageSystemRecentFileList::drawEvent(QPaintEvent* event, QPainter* p)
{
    ...
    for (int i = 0; i < d.m_recentFileActs.count(); ++i) 
    {
        ...
        StyleHintReturnThemeColor hintClientTextReturn("Ribbon", "GroupClientText");
